    ...
    return (
        <form onSubmit={handleSubmit} className="w-full relative group animate-fade-in anim-delay-100">
            ...
            <div className={`
                ...
                ${isLoading ? 'ring-2 ring-sky-500/20' : 'group-hover:border-sky-500/30 group-focus-within:ring-2 group-focus-within:ring-sky-500/20'}
            ...
            `}>
                <div className="pl-4 pr-3 text-[var(--text-muted)]">
                    {isLoading ? (
                        ...
                        <Search className="w-5 h-5 group-focus-within:text-sky-400 transition-colors" />
                    )}
                </div>
                ...
                <div className="flex items-center gap-2 pr-2">
                    ...
                    >
                        ...
                        {activeFilterCount > 0 && (
                            <span className="absolute -top-1 -right-1 w-4 h-4 bg-sky-500 rounded-full text-[9px] font-black text-white flex items-center justify-center">
                                {activeFilterCount}
                            </span>
                        )}
                    </button>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <input
                                ...
                                className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-[var(--text-main)] placeholder:text-slate-600 outline-none focus:border-sky-500/40 focus:ring-1 focus:ring-sky-500/20 transition-all"
                            />
                        </div>
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="relative">
                                <select
                                    ...
                                    className="w-full appearance-none bg-white/5 border border-white/10 rounded-xl px-3 py-2.5 text-sm text-[var(--text-main)] outline-none focus:border-sky-500/40 focus:ring-1 focus:ring-sky-500/20 transition-all cursor-pointer pr-8"
                                >
            ...
            {/* Search glow */}
            <div className="absolute -inset-1 bg-gradient-to-r from-sky-500/0 via-sky-500/0 to-emerald-500/0 rounded-2xl blur-lg transition-all duration-500 group-focus-within:from-sky-500/20 group-focus-within:via-sky-500/10 group-focus-within:to-emerald-500/20 -z-10" />
        </form>
    );
});
...
const Chip = ({ label, onRemove }) => (
    <span className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-sky-500/10 border border-sky-500/20 text-sky-300 text-[10px] font-black uppercase tracking-widest">
        {label}
        <button type="button" onClick={onRemove} className="hover:text-[var(--text-main)] transition-colors">
