...
const FEATURES = [
    { id: 'research', label: 'Academic Deep Research' },
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-xl bg-black/60">
            <div className="glass-card-heavy w-full max-w-lg p-10 border-white/10 shadow-2xl animate-modal-in relative">
                ...
                <button
                    onClick={onClose}
                    className="absolute top-6 right-6 p-2 rounded-xl text-slate-500 hover:text-[var(--text-main)] hover:bg-white/10 transition-all"
                >
                    <X className="w-5 h-5" />
                </button>
                ...
                <div className="text-center space-y-3 mb-10">
                    ...
                    <h3 className="text-2xl font-black text-[var(--text-main)] tracking-tight">How was your experience?</h3>
                ...
                <div className="space-y-8">
                                ...
                                <button
                                    ...
                                    className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                                        favoriteFeature === f.id
                                            ...
                                            : 'bg-white/5 border border-white/5 text-slate-400 hover:text-[var(--text-main)] hover:bg-white/10'
                                    }`}
                                >
                                    {f.label}
                                </button>
                    ...
                    <div className="space-y-3">
                        ...
                        <textarea
                            ...
                            rows={3}
                            className="w-full bg-white/5 border border-white/10 rounded-2xl px-5 py-4 text-sm text-[var(--text-main)] placeholder-slate-600 focus:outline-none focus:border-sky-500/30 resize-none transition-colors"
                        />
                    </div>
                ...
                <div className="flex gap-4 pt-8">
                    <button
                        onClick={onClose}
                        className="flex-1 py-4 rounded-2xl bg-white/5 text-slate-500 font-bold hover:bg-white/10 hover:text-[var(--text-main)] transition-all"
                    >
                        Skip
                    </button>
                    <button
