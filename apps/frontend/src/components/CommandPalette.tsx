...
import {
    Search, MessageSquare, Compass, Bookmark, FileText,
    FlaskConical, CreditCard,
    Settings, HelpCircle, Rocket, Sparkles, PenLine, Command, X,
    CornerDownLeft, Lightbulb
} from 'lucide-react';

// ─── Navigation items ────────────────────────────────────────────────────────
const NAV_ITEMS = [
    { id: 'feed', label: 'Home', icon: Rocket, group: 'Navigation', keywords: ['home', 'dashboard', 'feed', 'new research'] },
    { id: 'chat', label: 'Research Chat', icon: MessageSquare, group: 'Navigation', keywords: ['chat', 'ask', 'question', 'research'] },
    { id: 'history', label: 'History', icon: FileText, group: 'Navigation', keywords: ['history', 'investigations', 'sessions'] },
...
export interface CommandPaletteProps {
    ...
    papers: import('@/store/useResearchStore').Paper[];
    onSearch: (query: string) => void;
}

export const CommandPalette = ({ isOpen, onClose, onNavigate, papers = [], onSearch }: CommandPaletteProps) => {
    ...
    const handleSelect = useCallback((item) => {
        if (item.type === 'nav') {
            ...
            if (item.action === 'search') {
                ...
                const current = localStorage.getItem('tafiti-theme') || 'system';
                const next = order[(order.indexOf(current) + 1) % order.length] || 'system';
                localStorage.setItem('tafiti-theme', next);
                const mq = window.matchMedia('(prefers-color-scheme: dark)');
                const dark = next === 'dark' || (next === 'system' && mq.matches);
            ...
            <div className="relative w-full max-w-xl mx-4 glass-card-heavy border-white/10 shadow-2xl overflow-hidden animate-scale-in">
                ...
                <div className="flex items-center gap-3 px-5 py-4 border-b border-white/5">
                    ...
                    <input
                        ...
                        className="flex-1 bg-transparent text-[var(--text-main)] placeholder-slate-500 outline-none text-sm font-medium"
                            ...
                            return (
                                <button
                                    ...
                                    className={`w-full flex items-center gap-3 px-5 py-3 text-left transition-colors ${
                                        isSelected
                                            ? 'bg-sky-500/10 text-[var(--text-main)]'
                                            : 'text-slate-400 hover:bg-white/5 hover:text-[var(--text-main)]'
                                    }`}
                                >
                                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${
                                        isSelected ? 'bg-sky-500/20 text-sky-400' : 'bg-white/5 text-slate-500'
                                    }`}>
