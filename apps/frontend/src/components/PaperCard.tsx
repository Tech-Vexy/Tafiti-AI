            ...
            >
                ...
                <div className="flex items-start justify-between gap-4">
                    <div className="flex flex-wrap gap-2">
                        <span className="px-3 py-1 bg-sky-500/10 text-sky-300 rounded-full text-[11px] font-medium border border-sky-500/20">
                    ...
                    <button onClick={onClose} className="p-2 rounded-xl text-slate-500 hover:text-[var(--text-main)] hover:bg-white/10 transition-all shrink-0">
                        <X className="w-5 h-5" />
                    </button>
                </div>

                {/* Title */}
                <h2 className="text-lg sm:text-2xl font-black text-[var(--text-main)] leading-snug">{paper.title}</h2>
                ...
                <div className="flex flex-wrap gap-4 text-sm text-slate-400">
                    ...
                    {paper.journal && (
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[var(--text-main)] font-medium">{paper.journal}</span>
                        </div>
                    )}
                </div>
                ...
                {doiUrl && (
                    <a
                        ...
                        className="flex items-center gap-2 px-5 py-3 rounded-2xl bg-sky-500/10 border border-sky-500/20 text-sky-400 hover:bg-sky-500/20 transition-all text-sm font-medium w-fit"
                    >
    ...
    return (
        ...
        <div
            ...
            className={`
                ...
                ${isSelected
                    ? 'ring-2 ring-sky-500 bg-sky-500/[0.04] translate-y-[-2px] shadow-xl shadow-sky-500/10'
                    : 'hover:bg-white/[0.04]'}
            `}
        >
            {/* Background Decorative Element */}
            <div className={`absolute -top-10 -right-10 w-40 h-40 bg-sky-500/10 blur-[80px] rounded-full transition-opacity duration-700 ${isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`} />

            {/* Top Section: Badges & Actions */}
            <div className="flex justify-between items-start mb-5 sm:mb-8 relative z-10">
                <div className="flex items-center gap-2 flex-wrap">
                    <div className="px-3 py-1 bg-sky-500/10 text-sky-300 rounded-full text-[11px] font-medium border border-sky-500/20">
                        {paper.source || 'OpenAlex'}
                    </div>
                    {isOA ? (
                ...
                <div className="flex gap-2 sm:gap-3">
                    <button
                        ...
                        className={`p-2.5 rounded-xl transition-all duration-300 border ${showImpact ? 'text-sky-400 bg-sky-500/10 border-sky-500/20' : 'text-slate-500 bg-white/5 border-white/5 hover:text-sky-300 hover:border-sky-500/20'}`}
                        title="AI Impact Explainer"
                    >
                        {isImpactLoading ? (
                            <Loader2 className="w-4 h-4 animate-spin" />
                        ) : (
                            <Sparkles className={`w-4 h-4 ${showImpact ? 'fill-sky-400' : ''}`} />
                        )}
                    </button>
                    {onGraph && (
                        <button
                            onClick={(e) => { e.stopPropagation(); onGraph(paper); }}
                            className="p-2.5 rounded-xl transition-all duration-300 border text-sky-400 bg-sky-500/5 border-sky-500/10 hover:bg-sky-500/15 hover:border-sky-500/30"
                    ...
                    <button
                        ...
                        className="p-2.5 rounded-xl transition-all duration-300 border text-sky-400 bg-sky-500/5 border-sky-500/10 hover:bg-sky-500/10 hover:border-sky-500/30"
                        title="Send to Notes"
                    >
                    ...
                    <button
                        onClick={handleSave}
                        className={`p-2.5 rounded-xl transition-all duration-300 border ${localSaved ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' : 'text-slate-500 bg-white/5 border-white/5 hover:text-[var(--text-main)] hover:border-white/10'}`}
                    >
            ...
            <div className="space-y-5 relative z-10">
                <h3 className="text-base font-bold leading-snug text-[var(--text-main)] group-hover:text-sky-300 transition-colors duration-300">
                    {paper.title}
                </h3>

                {showImpact && impact ? (
                    <div className="animate-slide-up bg-sky-500/5 rounded-2xl p-4 border border-sky-500/10 space-y-3">
                        <div className="flex items-center justify-between">
                            <span className="text-xs font-medium text-sky-400">Impact · {impact.career_field}</span>
                            <div className="flex items-center gap-1">
                                <Star className="w-3 h-3 text-amber-400 fill-amber-400" />
                                <span className="text-xs font-semibold text-[var(--text-main)]">{impact.relevance_score}/10</span>
                            </div>
                        </div>
                        <p className="text-xs text-[var(--text-dim)] leading-relaxed italic border-l-2 border-sky-500/30 pl-3">
                            "{impact.impact_summary}"
                        </p>
                        <div className="space-y-1 pt-1">
                            <span className="text-[11px] font-semibold uppercase tracking-wide text-slate-500">Key Takeaway</span>
                            <p className="text-xs text-[var(--text-main)] font-medium leading-snug">{impact.key_takeaway}</p>
                        </div>
                    </div>
                ) : (
                    <>
                        <div className="flex flex-wrap items-center gap-3 sm:gap-5 text-xs text-slate-500">
                            <div className="flex items-center gap-2 group/author cursor-help">
                                <div className="w-6 h-6 rounded-full bg-white/5 flex items-center justify-center border border-white/5 group-hover/author:border-sky-500/30 transition-colors">
                                    <User className="w-3 h-3 group-hover/author:text-sky-400 transition-colors" />
                                </div>
                                <span className="group-hover/author:text-[var(--text-dim)] transition-colors">{paper.authors[0] || 'Unknown'} et al.</span>
                            </div>
                            <div className="flex items-center gap-2">
                                <Calendar className="w-3 h-3 text-sky-500/50" />
                                <span>{paper.year}</span>
                            </div>
                            <div className="flex items-center gap-2">
            ...
            <div className="mt-5 sm:mt-8 pt-4 sm:pt-6 border-t border-white/5 flex items-center justify-between relative z-10">
                <div className={`flex items-center gap-1.5 transition-all duration-300 ${isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
                    <span className="text-xs font-medium text-sky-400">Selected</span>
