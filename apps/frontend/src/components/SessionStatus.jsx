        ...
        return (
            <div className="glass-card p-4">
                ...
                <button onClick={startResearch} disabled={actionLoading === 'start'}
                    className="w-full px-3 py-2 text-xs flex items-center justify-center gap-1 rounded-lg border border-indigo-500/20 bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500/20 transition-colors disabled:opacity-50">
        ...
        <div className="glass-card p-4 space-y-4">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-white uppercase tracking-wider">{session.status}</span>
                </div>
            ...
            <div className="grid grid-cols-4 gap-2">
                {[
                    ...
                    { label: 'Checkpoints', value: session.checkpoints || 0, icon: Shield, color: 'purple' },
                    ...
                    {auditLog.length === 0 ? (
                        ...
                        auditLog.slice(0, 10).map((entry, i) => (
                            <div key={i} className="flex items-center gap-2 text-[10px] text-slate-500">
                                ...
                                <span className="text-white">{entry.from_status}</span>
                                <span className="text-indigo-400">{'>'}</span>
                                <span className="text-white">{entry.to_status}</span>
                            </div>
                        ))
                    )}
                </div>
